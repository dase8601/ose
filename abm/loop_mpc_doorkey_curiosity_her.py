"""
abm/loop_mpc_doorkey_curiosity_her.py — Run 7: Curiosity + HER combined.

Combines the two strongest individual interventions:
  1. Curiosity-driven OBSERVE (Run 4): novelty-maximizing action selection
     drives the world model into rare transitions it never sees during random walk.
  2. HER goal buffer (Run 5): every episode end is an EBM training pair,
     giving thousands of positives instead of ~23 from successes alone.

Scientific question: does the combination of richer OBSERVE data (curiosity)
and richer EBM training signal (HER) together fix the 0% result when neither
alone was sufficient? If Run 4 and Run 5 each fail but Run 7 succeeds, it
confirms the two problems compound and both must be solved simultaneously.
Saves to: results/doorkey_mpc/checkpoint_curiosity_her.pt
"""
...
def run_doorkey_curiosity_her_loop(
    condition: str = "curiosity_her",
    device: str = "cuda",
    ...
    if condition != "curiosity_her":
        raise ValueError(f"loop_mpc_doorkey_curiosity_her supports: curiosity_her — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_curiosity_her.py only supports env_type='doorkey'.")
    ...
    logger.info(
        f"[{condition.upper()}] Run 7: Curiosity + HER — device={device}, "
    ...
    while env_step < max_steps:
        ...
        if in_observe:
            ...
            # Curiosity-driven action selection once world model is warm
...
run_abm_loop = run_doorkey_curiosity_her_loop
