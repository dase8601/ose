"""
abm/loop_mpc_doorkey_run15c.py — Run 15c: Pure symbolic state, no visual encoder

Why this run:
  Runs 12-14 simultaneously debug encoder quality + predictor quality +
  EBM quality + planning architecture. We don't know which is failing.
  This run isolates the planner: give CEM+EBM a perfect world model and
  ask whether it can solve DoorKey at all.

  If 15c fails: the planner structure itself (CEM horizon, EBM loss, elite
  selection) is wrong — fix before touching encoders again.
  If 15c works: the architecture is correct, all prior failures are encoder
  quality — validate the LeCun approach and move to harder envs.

State representation (5-dim, no visual encoder):
  [has_key, door_open, agent_x/5, agent_y/5, agent_dir/3]

  All values in [0, 1]. Agent direction 0-3 → normalized to [0, 1/3, 2/3, 1].
  Consecutive states differ by meaningful amounts (0.2 per tile move,
  0.33 per turn) — cos_sim between different states is ~0.85-0.98,
  giving both the predictor and EBM real signal to work with.

  No encoder download, no discrimination test, no GPU warmup needed.
  Seeder reads symbolic state directly from env.unwrapped.
...
def _seed_scripted(key_buf, door_buf, goal_buf, seed_buf, post_door_neg_buf,
    ...
    logger.info(f"[RUN15C] Scripted seeding: {n_eps} episodes (5-dim symbolic)…")
    successes = 0
    ...
    logger.info(
        f"[RUN15C] Seed done: {successes}/{n_eps} | seed_buf={len(seed_buf)} | "
...
def _eval_run15c(mpc, key_buf, door_buf, goal_buf, device, seed_offset=1000, n_eps=EVAL_N_EPS):
    if mpc is None:
        return 0.0
    successes = 0
    for ep in range(n_eps):
...
def run_doorkey_run15c_loop(
    condition: str = "symbolic_only",
    device: str = "cuda",
    ...
    if condition != "symbolic_only":
        raise ValueError(f"loop_mpc_doorkey_run15c supports: symbolic_only — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run15c only supports env_type='doorkey'.")

    torch.manual_seed(seed); np.random.seed(seed)
    _observe_steps = observe_steps if observe_steps is not None else DEFAULT_OBSERVE

    logger.info(
        f"[RUN15C] Pure symbolic state ({FEATURE_DIM}-dim) + H={CEM_HORIZON} — no visual encoder | "
    ...
    while env_step < max_steps:
        ...
        if pred_loss is not None:
            ...
            if _train_ebm(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf, post_door_neg_buf, buf, device):
                ...
                if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc is not None:
                    ...
                    logger.info(f"[RUN15C] EBM activated | key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")
            ...
            if mpc is None and len(buf) >= PRED_WARMUP:
                ...
                logger.info(f"[RUN15C] CEM ready (H={CEM_HORIZON}, dim={FEATURE_DIM}) | "
                            f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")

        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            logger.info(
                f"[RUN15C] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} "
        ...
        if env_step % EVAL_INTERVAL < n_envs:
            sr = _eval_run15c(mpc, key_buf, door_buf, goal_buf, device,
                              seed_offset=9000 + env_step, n_eps=EVAL_N_EPS)
            elapsed = time.time() - t0
            logger.info(
                f"[RUN15C] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN15C] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN15C] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run15c_loop
