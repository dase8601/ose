"""
abm/loop_mpc_doorkey_run24.py — Run 24: Frozen predictor + scripted BFS stage 3

Why this run:
  Run 23 confirmed stage 3 was the sole initial bottleneck (goal grew past 226,
  87% stage-3 conversion when door opens).  But a new bottleneck emerged:
  pred_ewa degraded from 0.011 → 0.006 during ACT as the main replay buffer
  filled with goal-directed data, diluting the scripted seed_buf.  Fewer
  accurate predictions → CEM stages 0/1 weaken → door-opening rate drops
  from 1/1k steps to 1/10k steps → success stays at 5-15% instead of climbing.

  Run 24 fixes this with one change: freeze the predictor at the end of OBSERVE.
  Train it for 40k OBSERVE steps (same as all prior runs), then lock it when
  ACT begins.  The EBM continues training (softplus, non-saturating) throughout
  ACT so goal discrimination keeps improving.  The CEM uses a fixed peak-quality
  predictor for all 160k ACT steps.

  Stage 3 remains scripted BFS (same as Run 23) — this is still a diagnostic.
  If freezing the predictor sustains the early door-opening rate (~1/1k steps)
  throughout ACT, success should climb well past 20% and potentially above 42%.
...
CEM_HORIZON            = 8    # stages 0/1 only — stage 2 uses scripted BFS
CEM_SAMPLES            = 512
CEM_ELITES             = 64
CEM_ITERS              = 5
...
def _scripted_stage3_action(uw) -> int:
    """Navigate to the goal cell using BFS. Only called when door is already open."""
    pos, d = tuple(map(int, uw.agent_pos)), int(uw.agent_dir)
    gp = _find_cell(uw, "goal")
    if gp is None:
    ...
    fwd = (pos[0]+_DIR_TO_VEC[d][0], pos[1]+_DIR_TO_VEC[d][1])
    if fwd == gp:
        return 2  # forward = step onto goal
    path = _bfs(uw, pos, gp, allow_open_door=True)
...
def _seed_scripted(key_buf, door_buf, goal_buf, seed_buf, post_door_neg_buf,
    ...
    logger.info(f"[RUN24] Scripted seeding: {n_eps} episodes (5-dim symbolic)…")
    successes = 0
    ...
    logger.info(
        f"[RUN24] Seed done: {successes}/{n_eps} | seed_buf={len(seed_buf)} | "
...
def _eval_run24(mpc, key_buf, door_buf, goal_buf, device,
                seed_offset=1000, n_eps=EVAL_N_EPS):
    """Stages 0/1: CEM+EBM.  Stage 2: scripted BFS."""
    if mpc is None:
        return 0.0
    successes = 0
    for ep in range(n_eps):
        ...
        while not done and ep_steps < EP_MAX_STEPS:
            ...
            if current_stage == 2:
                action = _scripted_stage3_action(uw)
            else:
...
def run_doorkey_run24_loop(
    condition: str = "symbolic_frozen_pred_stage3",
    device: str = "cuda",
    ...
    if condition != "symbolic_frozen_pred_stage3":
        raise ValueError(f"loop_mpc_doorkey_run23 supports: symbolic_frozen_pred_stage3 — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run23 only supports env_type='doorkey'.")

    torch.manual_seed(seed); np.random.seed(seed)
    _observe_steps = observe_steps if observe_steps is not None else DEFAULT_OBSERVE

    logger.info(
        f"[RUN24] Symbolic (5-dim) | CEM+EBM stages 0/1 | scripted BFS stage 2 | "
    ...
    while env_step < max_steps:
        ...
        if in_observe:
            ...
            # Update stages and goals for stages 0/1 only
            ...
            # Stage 0/1: CEM+EBM batch planning
            # Stage 2: per-env scripted BFS
            ...
            for i in s2_idx:
                uw = envs.envs[i].unwrapped
                actions[i] = _scripted_stage3_action(uw)
        ...
        # Predictor: train during OBSERVE only — freeze at ACT start to prevent degradation
        if in_observe:
        ...
        # EBM: train throughout (hinge OBSERVE → softplus ACT), independent of predictor
        if len(buf) >= PRED_WARMUP:
            ...
            if train_fn(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf, post_door_neg_buf, buf, device):
                ...
                if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc is not None:
                    ...
                    logger.info(f"[RUN24] EBM activated at step {env_step} | "
        ...
        if mpc is None and len(buf) >= PRED_WARMUP:
            ...
            logger.info(f"[RUN24] CEM ready (H={CEM_HORIZON} stages 0/1 only, scripted stage 2) | "
                        f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")
        ...
        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            ...
            logger.info(
                f"[RUN24] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} "
        ...
        if env_step % EVAL_INTERVAL < n_envs:
            sr = _eval_run24(mpc, key_buf, door_buf, goal_buf, device,
                              seed_offset=9000 + env_step, n_eps=EVAL_N_EPS)
            elapsed = time.time() - t0
            logger.info(
                f"[RUN24] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN24] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN24] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run24_loop
