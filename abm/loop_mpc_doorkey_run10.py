"""
abm/loop_mpc_doorkey_run10.py — Run 10: Protected seed buffer

Diagnosis from Run 9:
  _seed_scripted() only pushed images to key_buf/door_buf/goal_buf (EBM training).
  It never pushed (obs, action, next_obs) to buf_lew (WM training).
  The world model was trained on 80k OBSERVE steps of curiosity exploration —
  entirely left-half transitions — same OOD problem as Runs 1–8.
  The seeder gave EBM the right images but gave the WM nothing.

  Evidence: door grew 201→211 during ACT (CEM opens door ~10x) but goal
  stayed frozen at 200 (seed). After opening the door, CEM hallucinates
  the right half of the grid because the WM has zero training data there.

Fix: push seeded transitions to a separate seed_buf (ReplayBuffer, never
overwritten). Every WM training step mixes 50% seed_buf + 50% buf_lew
(online). Post-door dynamics are permanently represented in WM training
throughout all 200k steps.

Everything else identical to Run 9 (H=8, subgoals, curiosity, HER).
...
SEED_BUF_CAPACITY  = 20_000   # protected: seeded transitions never overwritten
GOAL_CAPACITY      = 1_024
HER_CAPACITY       = 4_096
EVAL_INTERVAL      = 5_000
EVAL_N_EPS         = 10
...
# ── Scripted seeder — now populates seed_buf for WM training ──────────────

def _seed_scripted(
    key_buf: GoalImageBuffer,
    door_buf: GoalImageBuffer,
    goal_buf: GoalImageBuffer,
    seed_buf: ReplayBuffer,          # NEW: permanent WM training buffer
    n_eps: int = N_SEED_EPS,
    seed: int = 7777,
) -> None:
    """
    BFS-guided seeder. Populates:
      - key_buf / door_buf / goal_buf: EBM goal images (200+ each)
      - seed_buf: (obs, action, next_obs) transitions from complete episodes,
                  covering both halves of the grid. Never overwritten by OBSERVE.
    """
    logger.info(f"[RUN10] Scripted seeding: {n_eps} episodes…")
    successes = 0
    total_transitions = 0
    ...
    for ep in range(n_eps):
        ...
        while not done and steps < EP_MAX_STEPS:
            ...
            # Push to protected WM training buffer
    ...
    logger.info(
        f"[RUN10] Seed done: {successes}/{n_eps} succeeded | "
...
def _train_lewm_mixed(
    ...
    """Train WM for n_steps, mixing 50% seeded + 50% online data."""
...
def run_doorkey_run10_loop(
    condition: str = "protected_seed",
    device: str = "cuda",
    ...
    if condition != "protected_seed":
        raise ValueError(f"loop_mpc_doorkey_run10 supports: protected_seed — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run10.py only supports env_type='doorkey'.")
    ...
    logger.info(
        f"[RUN10] Protected seed buffer + H={CEM_HORIZON} + subgoals + curiosity + HER | "
    ...
    seed_buf = ReplayBuffer(capacity=SEED_BUF_CAPACITY)   # protected — never overwritten

    _seed_scripted(key_buf, door_buf, goal_buf, seed_buf, n_eps=N_SEED_EPS, seed=seed + 999)
    ...
    while env_step < max_steps:
        ...
        if in_observe:
            ...
            if ssl_loss_val is not None:
                ...
                if mpc is None:
                    ...
                    logger.info(
                        f"[RUN10] CEM ready (H={CEM_HORIZON}) | "
                        f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}"
                    )
            ...
            if ssl_loss_val is not None:
                ...
                if _train_ebm(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf, buf_lew, lewm, device):
                    ...
                    if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active:
                        mpc.set_ebm(ebm)
                        ebm_active = True
                        logger.info(f"[RUN10] EBM activated in ACT")

        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            logger.info(
                f"[RUN10] heartbeat step={env_step} | buf={len(buf_lew)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} her={len(her_buf)} | "
        ...
        if env_step % EVAL_INTERVAL < n_envs:
            ...
            logger.info(
                f"[RUN10] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN10] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN10] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run10_loop
