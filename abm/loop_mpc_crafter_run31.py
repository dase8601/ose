...
Condition:   lewm_crafter_hierarchy_v2_v2
...
RunPod:
  ...
  python abm_experiment.py --loop-module abm.loop_mpc_crafter_run31 \\
    --condition lewm_crafter_hierarchy_v2_v2 --device cuda --env crafter \\
            ...
            for i in range(n_envs):
                if active_goal_z[i] is not None:
                    cos_i = float(F.cosine_similarity(
                        z_cur_t[i:i+1], active_goal_z[i], dim=-1
                    ).item())
                    progress = max(0.0, cos_i - prev_cos_sim[i])
