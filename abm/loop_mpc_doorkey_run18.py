"""
abm/loop_mpc_doorkey_run18.py — Run 18: Symbolic state + non-saturating EBM (softplus loss)

Why this run:
  Run 15c confirmed the CEM+EBM architecture works: pred_ewa stable at 0.010,
  stages 0/1 both growing, 83% door→exit conversion in the first 24k ACT steps.
  The only failure was EBM saturation at stage 3: once post_door_neg_buf fills
  to 5000 and hinge margins are satisfied, gradients → 0 and CEM gets uniform
  energy for exit navigation.

  Run 17 tested replacing stage-3 EBM with L2 cost in position space. It failed:
  only 1 success in 60k ACT steps vs 15c's 19 in 24k. Root cause: H=8 compound
  prediction errors make L2 position regression too inaccurate. EBM's holistic
  pattern-matching was better — it just needed to not saturate.

  Run 18 fixes saturation directly. The hinge loss clamp is replaced with softplus:
    Old: loss = clamp(margin + E_pos - E_neg, min=0)  ← gradient = 0 once margin met
    New: loss = softplus(E_pos - E_neg)                ← gradient = sigmoid(E_pos - E_neg)

  Softplus gradient is sigmoid(·) which approaches 0 only asymptotically (never
  exactly 0). The EBM keeps learning every step of the 200k run regardless of how
  well it has already separated positives from negatives.

  All other architecture is identical to Run 15c.
...
def _seed_scripted(key_buf, door_buf, goal_buf, seed_buf, post_door_neg_buf,
    ...
    logger.info(f"[RUN18] Scripted seeding: {n_eps} episodes (5-dim symbolic)…")
    successes = 0
    ...
    logger.info(
        f"[RUN18] Seed done: {successes}/{n_eps} | seed_buf={len(seed_buf)} | "
...
def _train_ebm_softplus(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf,
    ...
    """Non-saturating EBM training via softplus loss instead of hinge clamp.

    softplus(E_pos - E_neg) has gradient sigmoid(E_pos - E_neg) which
    approaches 0 only asymptotically — never exactly 0 regardless of how
    well the EBM separates goal from non-goal states.
    """
...
def _eval_run18(mpc, key_buf, door_buf, goal_buf, device, seed_offset=1000, n_eps=EVAL_N_EPS):
    if mpc is None:
        return 0.0
    successes = 0
    for ep in range(n_eps):
...
def run_doorkey_run18_loop(
    condition: str = "symbolic_bce_ebm",
    device: str = "cuda",
    ...
    if condition != "symbolic_bce_ebm":
        raise ValueError(f"loop_mpc_doorkey_run18 supports: symbolic_bce_ebm — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run18 only supports env_type='doorkey'.")

    torch.manual_seed(seed); np.random.seed(seed)
    _observe_steps = observe_steps if observe_steps is not None else DEFAULT_OBSERVE

    logger.info(
        f"[RUN18] Symbolic (5-dim) + softplus EBM (non-saturating) | "
    ...
    while env_step < max_steps:
        ...
        if pred_loss is not None:
            ...
            if _train_ebm_softplus(ebm, opt_ebm, key_buf, door_buf, goal_buf,
                                   her_buf, post_door_neg_buf, buf, device):
                ebm_train_count += 1
                if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc is not None:
                    mpc.set_ebm(ebm); ebm_active = True
                    logger.info(f"[RUN18] EBM activated (softplus, all stages) | "
            ...
            if mpc is None and len(buf) >= PRED_WARMUP:
                ...
                logger.info(f"[RUN18] CEM ready (H={CEM_HORIZON}, dim={FEATURE_DIM}) | "
                            f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")

        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            logger.info(
                f"[RUN18] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} "
                f"post_neg={len(post_door_neg_buf)} | "
                f"pred_ewa={pred_ewa or 0.0:.4f} | "
                f"ebm={'ON(softplus)' if ebm_active else f'training({ebm_train_count})'} | {time.time()-t0:.0f}s"
            )

        if env_step % EVAL_INTERVAL < n_envs:
            sr = _eval_run18(mpc, key_buf, door_buf, goal_buf, device,
                             seed_offset=9000 + env_step, n_eps=EVAL_N_EPS)
            elapsed = time.time() - t0
            logger.info(
                f"[RUN18] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN18] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN18] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run18_loop
