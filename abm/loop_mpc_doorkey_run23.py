"""
abm/loop_mpc_doorkey_run23.py — Run 23: Scripted BFS oracle for stage 3 (diagnostic)

Why this run:
  Runs 15c, 18, and 20 all stall at goal≈224–226 despite different EBM loss
  functions and pred_ewa rising to 0.0144.  Run 21 tests whether H=12 fixes it.

  This run answers the highest-value diagnostic question first:
    Is stage 3 (door→exit) the ONLY bottleneck?

  Method: keep CEM+EBM (hinge→softplus two-phase, identical to Run 20) for
  stages 0 and 1 — the world model does the hard planning (key pickup, door
  opening).  For stage 2 only, replace CEM with the existing BFS scripted
  policy (_scripted_action navigates to the goal cell).

  Interpretation:
    If this run beats 42%: stage 3 IS the sole ceiling.  Run 21 (H=12) and
      any further planner tuning are the right path.
    If this run still stalls at ~25%: stages 0/1 have a hidden bottleneck
      that the buffer counts are masking — rethink before spending more pods.

  This is a diagnostic, not a proof.  It does not publish as "planning beats
  RL" but it validates whether the research direction is worth continuing.
...
def _seed_scripted(key_buf, door_buf, goal_buf, seed_buf, post_door_neg_buf,
    ...
    logger.info(f"[RUN23] Scripted seeding: {n_eps} episodes (5-dim symbolic)…")
    successes = 0
    ...
    logger.info(
        f"[RUN23] Seed done: {successes}/{n_eps} | seed_buf={len(seed_buf)} | "
...
def _eval_run23(mpc, key_buf, door_buf, goal_buf, device,
...
def run_doorkey_run23_loop(
    condition: str = "symbolic_scripted_stage3",
    device: str = "cuda",
    ...
    if condition != "symbolic_scripted_stage3":
        raise ValueError(f"loop_mpc_doorkey_run23 supports: symbolic_scripted_stage3 — got: {condition}")
    if env_type != "doorkey":
    ...
    logger.info(
        f"[RUN23] Symbolic (5-dim) | CEM+EBM stages 0/1 | scripted BFS stage 2 | "
    ...
    while env_step < max_steps:
        ...
        if pred_loss is not None:
            ...
            if train_fn(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf, post_door_neg_buf, buf, device):
                ...
                if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc is not None:
                    ...
                    logger.info(f"[RUN23] EBM activated at step {env_step} | "
            ...
            if mpc is None and len(buf) >= PRED_WARMUP:
                ...
                logger.info(f"[RUN23] CEM ready (H={CEM_HORIZON} stages 0/1 only, scripted stage 2) | "
                            f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")
        ...
        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            ...
            logger.info(
                f"[RUN23] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} "
        ...
        if env_step % EVAL_INTERVAL < n_envs:
            sr = _eval_run23(mpc, key_buf, door_buf, goal_buf, device,
                              seed_offset=9000 + env_step, n_eps=EVAL_N_EPS)
            elapsed = time.time() - t0
            logger.info(
                f"[RUN23] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN23] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN23] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run23_loop
