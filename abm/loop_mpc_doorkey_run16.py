"""
abm/loop_mpc_doorkey_run16.py — Run 16: V-JEPA 2.1 + scaled symbolic + delayed EBM

Combines the two fixes from Run 15a and Run 15b:

  Run 15b fix (scaling): SYM_SCALE=10 so symbolic dims contribute ~5% of
    cosine signal (vs 0.5% unscaled). One-tile move → cos_sim≈0.980 instead
    of 0.9998. pred_ewa started at 0.010 before EBM fired.

  Run 15b bug (early EBM): EBM activated during OBSERVE (~step 6k) while
    predictor was still early in training. EBM gradients conflicted with
    predictor gradients — pred_ewa decayed from 0.010 to 0.001 by step 48k.
    Run killed at step 48k (OBSERVE not yet complete).

  Run 15a fix (delayed EBM): EBM training gated to `if not in_observe:` —
    EBM starts only when OBSERVE ends. Adapter/predictor stabilize for 80k
    steps first. EBM then trains on a stable representation.

  Run 16 = 15b's SYM_SCALE=10 + 15a's `if not in_observe:` gate.
    Expected: pred_ewa stable ~0.009-0.020 at OBSERVE end (as in 15c's
    symbolic baseline). EBM then trains on a predictor that actually
    learned position/key/door dynamics.

Architecture:
  Frozen V-JEPA 2.1 → 768-dim visual (frozen, L2-norm)
  concat [has_key×10, door_open×10, x/5×10, y/5×10] → 772-dim augmented
...
def _load_vjepa2(device: str) -> nn.Module:
    ...
    logger.info("[RUN16] Loading V-JEPA 2.1 ViT-Base-384 (frozen)...")
    ...
    if not ckpt_path.exists():
        logger.info("[RUN16] Downloading checkpoint...")
    ...
    logger.info(f"[RUN16] V-JEPA 2.1 loaded — {n_params:.1f}M params | FEATURE_DIM={FEATURE_DIM} (vis={VIS_DIM}+sym={SYM_DIM}×{SYM_SCALE})")
    return encoder
...
def _get_symbolic(uw) -> np.ndarray:
    """4-dim symbolic state scaled by SYM_SCALE."""
...
def _encode_aug_single(model: nn.Module, img_np: np.ndarray, uw, device: str) -> np.ndarray:
    ...
    z_vis = _encode_visual(model, img_np, device).cpu().numpy()[0]
    sym   = _get_symbolic(uw)
    return np.concatenate([z_vis, sym])
...
def _doorkey_discrimination_test(model: nn.Module, device: str) -> float:
    ...
    logger.info(f"[RUN16] Discrimination test: visual cos_sim={mean_sim:.4f} (symbolic scaling is the fix)")
    if mean_sim > 0.95:
        logger.warning(f"[RUN16] WARNING: visual cos_sim={mean_sim:.4f} > 0.95 — symbolic dims at scale {SYM_SCALE} compensate")
    return mean_sim
...
def _seed_scripted(
    ...
    logger.info(f"[RUN16] Scripted seeding: {n_eps} episodes…")
    successes = 0
    ...
    logger.info(
        f"[RUN16] Seed done: {successes}/{n_eps} | seed_buf={len(seed_buf)} | "
...
def _eval_run16(mpc, vjepa2, key_buf, door_buf, goal_buf, device, seed_offset=1000, n_eps=EVAL_N_EPS):
    if mpc is None:
        return 0.0
    successes = 0
    for ep in range(n_eps):
...
def run_doorkey_run16_loop(
    condition: str = "vjepa2_symbolic_scaled_late_ebm",
    device: str = "cuda",
    ...
    if condition != "vjepa2_symbolic_scaled_late_ebm":
        raise ValueError(f"loop_mpc_doorkey_run16 supports: vjepa2_symbolic_scaled_late_ebm — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run16 only supports env_type='doorkey'.")

    torch.manual_seed(seed); np.random.seed(seed)
    _observe_steps = observe_steps if observe_steps is not None else DEFAULT_OBSERVE

    logger.info(
        f"[RUN16] V-JEPA 2.1 + sym×{SYM_SCALE} ({FEATURE_DIM}d) + H={CEM_HORIZON} + delayed EBM | "
    ...
    while env_step < max_steps:
        ...
        if pred_loss is not None:
            ...
            # EBM trains only after OBSERVE ends — predictor is stable by then
            if not in_observe:
                if _train_ebm(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf, post_door_neg_buf, buf, device):
                    ...
                    if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc is not None:
                        ...
                        logger.info(f"[RUN16] EBM activated (step={env_step}) | "
            ...
            if mpc is None and len(buf) >= PRED_WARMUP:
                ...
                logger.info(f"[RUN16] CEM ready (H={CEM_HORIZON}) | key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")

        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            logger.info(
                f"[RUN16] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} "
        ...
        if env_step % EVAL_INTERVAL < n_envs:
            sr = _eval_run16(mpc, vjepa2, key_buf, door_buf, goal_buf, device,
                             seed_offset=9000 + env_step, n_eps=EVAL_N_EPS)
            elapsed = time.time() - t0
            logger.info(
                f"[RUN16] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN16] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN16] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run16_loop
