"""
abm/loop_mpc_doorkey_run17.py — Run 17: Symbolic + L2 goal distance for stage 3

Fix over Run 15c:
  Run 15c used EBM for all three stages. Stages 0 (key) and 1 (door) worked
  well — key and door buffers grew throughout ACT. Stage 2 (exit) stalled:
  goal grew 200→224 in the first 24k ACT steps then froze for the remaining
  136k steps. Peak success 15%, well below the 42% PPO baseline.

  Root cause: the contrastive hinge EBM for stage 3 saturates. Once
  E(goal_states) and E(non_goal_states) satisfy the hinge margin, gradients
  go to zero and CEM receives uniform energy across all right-half positions.
  The 15% peak came from the EBM before it saturated; everything after was
  the agent stumbling onto the exit by luck.

Fix:
  For stage 2 (exit navigation) only, replace the EBM cost with direct L2
  distance in position space:

    cost(z_H) = (z_H[2] - z_goal[2])^2 + (z_H[3] - z_goal[3])^2

  where dims 2 and 3 are agent_x/5 and agent_y/5. This signal:
    - Never saturates (continuous, always provides gradient)
    - Is provably correct given a perfect predictor
    - Requires no training — uses goal_buf sample's position dims directly

  Stages 0 (key) and 1 (door) still use the EBM — goal positions vary by
  episode seed and the EBM generalizes better than L2 there.

  This directly tests: "Is the EBM for stage 3 the binding constraint?"
  If this run beats 42%, yes. If it still fails, CEM sampling is the issue.
...
def _cem_stage3_l2_batch(
    predictor: nn.Module,
    z_batch: torch.Tensor,       # (B, 5) current symbolic states
    goal_xy_batch: torch.Tensor, # (B, 2) target [x_goal/5, y_goal/5]
    device: str,
    n_samples: int = CEM_SAMPLES,
    horizon: int = CEM_HORIZON,
    n_elites: int = CEM_ELITES,
    n_iters: int = CEM_ITERS,
) -> np.ndarray:
    """
    Batched CEM for stage 3 using L2 distance to goal position.
    No EBM — cost is purely positional. Never saturates.
    Returns: best first actions (B,) as numpy int array.
    """
    B = z_batch.shape[0]
    probs = torch.ones(B, horizon, N_ACTIONS, device=device) / N_ACTIONS

    for _ in range(n_iters):
        # Sample (B, n_samples, H) action indices from per-timestep distributions
        probs_flat = probs.unsqueeze(1).expand(B, n_samples, horizon, N_ACTIONS) \
                         .reshape(B * n_samples * horizon, N_ACTIONS)
        actions = torch.multinomial(probs_flat, num_samples=1) \
                       .reshape(B, n_samples, horizon)          # (B, n_samples, H)
        a_oh = F.one_hot(actions, N_ACTIONS).float()            # (B, n_samples, H, N_ACTIONS)

        # Roll out predictor: flatten to (B*n_samples, 5) for one big batch call
        z = z_batch.unsqueeze(1).expand(B, n_samples, FEATURE_DIM) \
                   .reshape(B * n_samples, FEATURE_DIM)
        a_oh_seq = a_oh.reshape(B * n_samples, horizon, N_ACTIONS)

        with torch.no_grad():
            for t in range(horizon):
                z = predictor(z, a_oh_seq[:, t])               # (B*n_samples, 5)

        z_final = z.reshape(B, n_samples, FEATURE_DIM)         # (B, n_samples, 5)

        # L2 cost on position dims [2, 3]
        costs = ((z_final[:, :, 2] - goal_xy_batch[:, 0:1]) ** 2 +
                 (z_final[:, :, 3] - goal_xy_batch[:, 1:2]) ** 2)   # (B, n_samples)

        # Update distributions from elites
        elite_idx = costs.argsort(dim=1)[:, :n_elites]         # (B, n_elites)
        for b in range(B):
            elite_acts = actions[b, elite_idx[b]]               # (n_elites, H)
            for t in range(horizon):
                counts = torch.bincount(elite_acts[:, t], minlength=N_ACTIONS).float()
                probs[b, t] = (counts + 0.1) / (n_elites + 0.1 * N_ACTIONS)

    # Return first action of best-cost sequence per env
    best_idx = costs.argsort(dim=1)[:, 0]                      # (B,)
    best_acts = actions[torch.arange(B, device=device), best_idx, 0]
    return best_acts.cpu().numpy()


def _cem_stage3_l2_single(
    predictor: nn.Module,
    z_cur: torch.Tensor,    # (1, 5)
    goal_xy: torch.Tensor,  # (2,)
    device: str,
) -> int:
    acts = _cem_stage3_l2_batch(predictor, z_cur, goal_xy.unsqueeze(0), device)
    return int(acts[0])
...
def _seed_scripted(key_buf, door_buf, goal_buf, seed_buf, post_door_neg_buf,
    ...
    logger.info(f"[RUN17] Scripted seeding: {n_eps} episodes (5-dim symbolic)…")
    successes = 0
    ...
    logger.info(
        f"[RUN17] Seed done: {successes}/{n_eps} | seed_buf={len(seed_buf)} | "
...
def _eval_run17(mpc, predictor, key_buf, door_buf, goal_buf, device,
    ...
    for ep in range(n_eps):
        ...
        while not done and ep_steps < EP_MAX_STEPS:
            ...
            if new_stage != current_stage or goal_age >= GOAL_REFRESH_STEPS:
                ...
                if current_stage < 2:
                    b = key_buf if current_stage == 0 else door_buf
                    z_goal_np = b.sample(1)
                    z_goal = torch.from_numpy(z_goal_np).to(device) if z_goal_np is not None else None
                else:
                    z_goal_np = goal_buf.sample(1)
                    z_goal = torch.from_numpy(z_goal_np).to(device) if z_goal_np is not None else None
                goal_age = 0
            ...
            if current_stage < 2:
                ...
                if z_goal is not None:
                    # L2 distance to goal position — dims [2, 3]
                    goal_xy = z_goal[0, 2:4]
                    action = _cem_stage3_l2_single(predictor, z_t, goal_xy, device)
                else:
...
def run_doorkey_run17_loop(
    condition: str = "symbolic_l2_stage3",
    device: str = "cuda",
    ...
    if condition != "symbolic_l2_stage3":
        raise ValueError(f"loop_mpc_doorkey_run17 supports: symbolic_l2_stage3 — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run17 only supports env_type='doorkey'.")

    torch.manual_seed(seed); np.random.seed(seed)
    _observe_steps = observe_steps if observe_steps is not None else DEFAULT_OBSERVE

    logger.info(
        f"[RUN17] Symbolic ({FEATURE_DIM}-dim) + EBM stages 0/1 + L2 stage 2 | "
    ...
    while env_step < max_steps:
        ...
        if in_observe:
            ...
            # Update stages and goal vectors
            ...
            # Separate envs by stage for efficient batched planning
            s01_idx = [i for i in range(n_envs)
                       if active_stage[i] < 2 and active_goal_z[i] is not None]
            s2_idx  = [i for i in range(n_envs)
                       if active_stage[i] == 2 and active_goal_z[i] is not None]

            actions = envs.action_space.sample()  # fallback for any unhandled env

            # Stages 0/1: EBM-guided CEM (same as 15c)
            if mpc is not None and s01_idx:
                z_s01 = z_cur_t[s01_idx]
                g_s01 = torch.from_numpy(
                    np.stack([active_goal_z[i] for i in s01_idx])
                ).to(device)
                acts_s01 = mpc.plan_batch(z_s01, g_s01)
                for k, i in enumerate(s01_idx):
                    actions[i] = acts_s01[k]

            # Stage 2: L2 position CEM — no EBM
            if s2_idx:
                if not l2_stage3_logged:
                    logger.info(f"[RUN17] Stage-3 L2 CEM active (step={env_step}) — EBM bypassed for exit nav")
                    l2_stage3_logged = True
                z_s2 = z_cur_t[s2_idx]
                goal_xy = torch.from_numpy(
                    np.stack([active_goal_z[i][2:4] for i in s2_idx])
                ).to(device)
                acts_s2 = _cem_stage3_l2_batch(predictor, z_s2, goal_xy, device)
                for k, i in enumerate(s2_idx):
                    actions[i] = acts_s2[k]
        ...
        if pred_loss is not None:
            ...
            if _train_ebm(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf, post_door_neg_buf, buf, device):
                ...
                if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc is not None:
                    ...
                    logger.info(f"[RUN17] EBM activated (stages 0/1 only) | "
            ...
            if mpc is None and len(buf) >= PRED_WARMUP:
                ...
                logger.info(f"[RUN17] CEM ready (H={CEM_HORIZON}, dim={FEATURE_DIM}) | "
                            f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")

        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            logger.info(
                f"[RUN17] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} "
                f"post_neg={len(post_door_neg_buf)} | "
                f"pred_ewa={pred_ewa or 0.0:.4f} | "
                f"ebm={'ON(s0/1)' if ebm_active else f'training({ebm_train_count})'} | {time.time()-t0:.0f}s"
            )

        if env_step % EVAL_INTERVAL < n_envs:
            sr = _eval_run17(mpc, predictor, key_buf, door_buf, goal_buf, device,
                             seed_offset=9000 + env_step, n_eps=EVAL_N_EPS)
            elapsed = time.time() - t0
            logger.info(
                f"[RUN17] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN17] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN17] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run17_loop
