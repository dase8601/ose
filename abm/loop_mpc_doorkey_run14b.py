"""
abm/loop_mpc_doorkey_run14b.py — Run 14b: V-JEPA 2.1 + symbolic state augmentation

The problem diagnosed in Runs 12-13:
  pred_ewa ≈ 0.0000 throughout ACT. The predictor's cosine-similarity loss
  is near-zero because adjacent DoorKey frames have cos_sim ≈ 0.999+ in
  V-JEPA / DINOv2 space — both encoders are trained to ignore low-level
  pixel differences, but DoorKey state changes ARE low-level pixel changes
  in a synthetic grid. The predictor converges to "predict current = next"
  and CEM becomes EBM-biased random search, not planning.
...
def _load_vjepa2(device: str) -> nn.Module:
    ...
    logger.info("[RUN14B] Loading V-JEPA 2.1 ViT-Base-384 (frozen)...")
    ...
    if not ckpt_path.exists():
        logger.info(f"[RUN14B] Downloading checkpoint...")
    ...
    logger.info(f"[RUN14B] V-JEPA 2.1 loaded — {n_params:.1f}M params | FEATURE_DIM={FEATURE_DIM} (vis={VIS_DIM}+sym={SYM_DIM})")
    return encoder
...
def _get_symbolic(uw) -> np.ndarray:
    """4-dim symbolic state: [has_key, door_open, agent_x/5, agent_y/5]"""
...
def _doorkey_discrimination_test(model: nn.Module, device: str) -> float:
    ...
    logger.info(f"[RUN14B] Discrimination test: cross-seed visual cos_sim={mean_sim:.4f}")
    if mean_sim > 0.95:
        logger.warning(f"[RUN14B] WARNING: visual cos_sim={mean_sim:.4f} > 0.95 — symbolic augmentation is the fix for this.")
    return mean_sim
...
def _seed_scripted(
    ...
    logger.info(f"[RUN14B] Scripted seeding: {n_eps} episodes…")
    successes = 0
    ...
    logger.info(
        f"[RUN14B] Seed done: {successes}/{n_eps} | seed_buf={len(seed_buf)} | "
...
def _eval_run14b(mpc, vjepa2, key_buf, door_buf, goal_buf, device, seed_offset=1000, n_eps=EVAL_N_EPS):
    if mpc is None:
        return 0.0
    successes = 0
    for ep in range(n_eps):
...
def run_doorkey_run14b_loop(
    condition: str = "vjepa2_symbolic",
    device: str = "cuda",
    ...
    if condition != "vjepa2_symbolic":
        raise ValueError(f"loop_mpc_doorkey_run14b supports: vjepa2_symbolic — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run14b only supports env_type='doorkey'.")

    torch.manual_seed(seed); np.random.seed(seed)
    _observe_steps = observe_steps if observe_steps is not None else DEFAULT_OBSERVE

    logger.info(
        f"[RUN14B] V-JEPA 2.1 + symbolic aug ({FEATURE_DIM}d) + H={CEM_HORIZON} | "
    ...
    while env_step < max_steps:
        ...
        if pred_loss is not None:
            ...
            if _train_ebm(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf, post_door_neg_buf, buf, device):
                ...
                if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc is not None:
                    ...
                    logger.info(f"[RUN14B] EBM activated | key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")
            ...
            if mpc is None and len(buf) >= PRED_WARMUP:
                ...
                logger.info(f"[RUN14B] CEM ready (H={CEM_HORIZON}) | key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")

        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            logger.info(
                f"[RUN14B] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
        ...
        if env_step % EVAL_INTERVAL < n_envs:
            sr = _eval_run14b(mpc, vjepa2, key_buf, door_buf, goal_buf, device,
                              seed_offset=9000 + env_step, n_eps=EVAL_N_EPS)
            elapsed = time.time() - t0
            logger.info(
                f"[RUN14B] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN14B] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN14B] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run14b_loop
