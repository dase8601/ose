"""
abm/loop_mpc_doorkey_run26.py — Run 26: Short-horizon CEM (H=3) for stage 2

Why this run (HWM paper insight, arXiv 2604.03208):
  Run 24 proved the predictor is accurate enough to beat the 42% PPO baseline (peak
  50%) — but only because stage 2 used an oracle BFS to navigate door→exit.  That
  is not publishable.  Run 25 replaces BFS with a PPO actor-critic (learned), but
  mixes RL into an otherwise-pure world-model system.

  The HWM paper (Figure 6) shows that low-level predictors are most accurate at
  short horizons (H≤4) and degrade badly at H≥8 due to compound error.  DoorKey
  stage 2 (door→exit) is a short navigation problem — the goal is at most 4-5 cells
  away.  A receding-horizon CEM at H=3, replanning every step, stays in the
  accurate regime while covering arbitrary distances through repeated short hops.

  Run 26 fixes this purely within the world-model framework:
    • Stages 0/1: CEM H=8 + EBM (identical to Run 24 — proven effective)
    • Stage 2:   CEM H=3 + EBM, goal sampled from goal_buf each GOAL_REFRESH_STEPS
    • Predictor frozen at ACT start (same as Run 24 — prevents pred_ewa degradation)
    • No BFS oracle.  No PPO.  Pure world-model planning for all three stages.

  If peak > 42% PPO baseline this result is publishable as evidence that hierarchical
  short-horizon replanning with a frozen predictor generalises across all task stages.

Condition: symbolic_short_horizon_s2
Loop module: abm.loop_mpc_doorkey_run26
"""
...
def _seed_scripted(key_buf, door_buf, goal_buf, seed_buf, post_door_neg_buf,
    ...
    logger.info(f"[RUN26] Scripted seeding: {n_eps} episodes (5-dim symbolic)…")
    successes = 0
    ...
    logger.info(
        f"[RUN26] Seed done: {successes}/{n_eps} | seed_buf={len(seed_buf)} | "
...
def _eval_run26(mpc, mpc_s2, key_buf, door_buf, goal_buf, device,
                seed_offset=1000, n_eps=EVAL_N_EPS):
    """Stages 0/1: CEM H=8 + EBM.  Stage 2: CEM H=3 + EBM (short-horizon)."""
    if mpc is None:
        return 0.0
    successes = 0
    for ep in range(n_eps):
...
def run_doorkey_run26_loop(
    condition: str = "symbolic_short_horizon_s2",
    device: str = "cuda",
    ...
    if condition != "symbolic_short_horizon_s2":
        raise ValueError(f"loop_mpc_doorkey_run26 supports: symbolic_short_horizon_s2 — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run26 only supports env_type='doorkey'.")

    torch.manual_seed(seed); np.random.seed(seed)
    _observe_steps = observe_steps if observe_steps is not None else DEFAULT_OBSERVE

    logger.info(
        f"[RUN26] Symbolic (5-dim) | CEM H={CEM_HORIZON} stages 0/1 | "
    ...
    while env_step < max_steps:
        ...
        if len(buf) >= PRED_WARMUP:
            ...
            if train_fn(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf, post_door_neg_buf, buf, device):
                ...
                if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc is not None:
                    ...
                    mpc_s2.set_ebm(ebm)
                    ebm_active = True
                    logger.info(f"[RUN26] EBM activated at step {env_step} | "
        ...
        if mpc is None and len(buf) >= PRED_WARMUP:
            ...
            mpc_s2 = CEMPlanner(predictor, n_actions=N_ACTIONS, horizon=CEM_HORIZON_S2,
                                ...
                                device=device, distance="cosine")
            logger.info(
                f"[RUN26] CEM ready | stages 0/1: H={CEM_HORIZON} | stage 2: H={CEM_HORIZON_S2} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}"
            )
        ...
        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            ...
            logger.info(
                f"[RUN26] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} "
        ...
        if env_step % EVAL_INTERVAL < n_envs:
            sr = _eval_run26(mpc, mpc_s2, key_buf, door_buf, goal_buf, device,
                             seed_offset=9000 + env_step, n_eps=EVAL_N_EPS)
            elapsed = time.time() - t0
            logger.info(
                f"[RUN26] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN26] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN26] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run26_loop
