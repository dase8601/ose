"""
abm/loop_mpc_doorkey_run14a.py — Run 14a: V-JEPA 2.1 + trainable adapter layer

The problem diagnosed in Runs 12-13:
  pred_ewa ≈ 0.0000 throughout ACT. Adjacent DoorKey frames have
  cos_sim ≈ 0.999+ in V-JEPA space — predicting "z_{t+1} ≈ z_t" is
  near-optimal under cosine loss. CEM becomes EBM-biased random search.
...
def _load_vjepa2(device: str) -> nn.Module:
    ...
    logger.info("[RUN14A] Loading V-JEPA 2.1 ViT-Base-384 (frozen)...")
    ...
    if not ckpt_path.exists():
        logger.info(f"[RUN14A] Downloading checkpoint...")
    ...
    logger.info(f"[RUN14A] V-JEPA 2.1 loaded — {n_params:.1f}M params (frozen) | adapter_dim={ADAPTER_DIM}")
    return encoder
...
def _doorkey_discrimination_test(model: nn.Module, device: str) -> float:
    ...
    logger.info(f"[RUN14A] V-JEPA raw discrimination: cross-seed cos_sim={mean_sim:.4f} (adapter will fix this)")
    return mean_sim
...
def _seed_scripted(
    ...
    logger.info(f"[RUN14A] Scripted seeding: {n_eps} episodes (storing raw 768-dim)…")
    successes = 0
    ...
    logger.info(
        f"[RUN14A] Seed done: {successes}/{n_eps} | seed_buf={len(seed_buf)} | "
...
def _eval_run14a(mpc, vjepa2, adapter, key_buf, door_buf, goal_buf, device, seed_offset=1000, n_eps=EVAL_N_EPS):
    if mpc is None:
        return 0.0
    successes = 0
    for ep in range(n_eps):
...
def run_doorkey_run14a_loop(
    condition: str = "vjepa2_adapter",
    device: str = "cuda",
    ...
    if condition != "vjepa2_adapter":
        raise ValueError(f"loop_mpc_doorkey_run14a supports: vjepa2_adapter — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run14a only supports env_type='doorkey'.")

    torch.manual_seed(seed); np.random.seed(seed)
    _observe_steps = observe_steps if observe_steps is not None else DEFAULT_OBSERVE

    logger.info(
        f"[RUN14A] V-JEPA 2.1 (frozen 768-dim) + adapter ({RAW_DIM}→{ADAPTER_DIM}) + H={CEM_HORIZON} | "
    ...
    while env_step < max_steps:
        ...
        if pred_loss is not None:
            ...
            if _train_ebm(adapter, ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf,
                ...
                if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc is not None:
                    ...
                    logger.info(f"[RUN14A] EBM activated | key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")
            ...
            if mpc is None and len(buf) >= PRED_WARMUP:
                ...
                logger.info(f"[RUN14A] CEM ready (H={CEM_HORIZON}, adapter_dim={ADAPTER_DIM}) | "
                            f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")

        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            logger.info(
                f"[RUN14A] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} "
        ...
        if env_step % EVAL_INTERVAL < n_envs:
            sr = _eval_run14a(mpc, vjepa2, adapter, key_buf, door_buf, goal_buf,
                              device, seed_offset=9000 + env_step, n_eps=EVAL_N_EPS)
            elapsed = time.time() - t0
            logger.info(
                f"[RUN14A] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN14A] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN14A] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run14a_loop
