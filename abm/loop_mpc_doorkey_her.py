"""
abm/loop_mpc_doorkey_her.py — Run 5: Hindsight Experience Replay goal buffer.

Identical to loop_mpc_doorkey.py (Run 3) except:
  EBM receives two training signals instead of one:
    1. Standard: (z_success, z_goal) vs (z_random, z_goal) — as before
    2. HER:      (z_terminal, z_terminal) vs (z_random, z_terminal) — every
                 episode end is treated as a "goal achieved" for that final state.

Scientific question: does richer EBM training signal (more + HER pairs) fix 0%?
Run 3 had ~23 successes in 200k steps — nearly zero EBM positives. HER gives
positives from every one of the thousands of episode terminations instead.
Saves to: results/doorkey_mpc/checkpoint_her_goals.pt
"""
...
GOAL_CAPACITY      = 1_024
HER_CAPACITY       = 4_096          # larger — all episode ends go here
EVAL_INTERVAL      = 5_000
EVAL_N_EPS         = 10
...
EP_MAX_STEPS       = 300
EBM_MIN_GOALS      = 5
EBM_HER_MIN        = 20             # HER needs more samples before it's useful
...
def _train_ebm_step(
    ...
    """
    One EBM training step with two signals:
      1. Standard: (z_success, z_goal) vs (z_replay_random, z_goal)
      2. HER:      (z_her_end, z_her_end) vs (z_replay_random, z_her_end)

    HER uses the same terminal-state image as both the "achieved" state and
    the goal — the EBM learns: energy is low when the state IS the goal.
    """
    opt_ebm.zero_grad()
    total_loss = torch.tensor(0.0, device=device)
    n_terms = 0

    # Standard signal (only when we have actual successes)
    ...
    # HER signal: any terminal state is a "goal achieved" for itself
    if len(her_buf) >= EBM_HER_MIN and len(buf_lew) >= EBM_BATCH * 2:
        ...
        # z_her serves as both z_pos and z_goal (HER relabeling)
...
def run_doorkey_her_loop(
    condition: str = "her_goals",
    device: str = "cuda",
    ...
    if condition != "her_goals":
        raise ValueError(f"loop_mpc_doorkey_her supports: her_goals — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_her.py only supports env_type='doorkey'.")
    ...
    logger.info(
        f"[{condition.upper()}] Run 5: HER goal buffer — device={device}, "
    ...
    goal_buf = GoalImageBuffer(capacity=GOAL_CAPACITY)        # success-only → CEM/eval
    her_buf  = GoalImageBuffer(capacity=HER_CAPACITY)         # all terminals → EBM HER
    ...
    while env_step < max_steps:
        ...
        if in_observe:
            ...
            for i in range(n_envs):
                ...
                if dones[i]:
                    # HER: every terminal state is a "goal" for itself
            ...
            if len(buf_lew) >= LEWM_WARMUP:
                ...
                if _train_ebm_step(ebm, opt_ebm, goal_buf, her_buf, buf_lew, lewm, device):
                    ebm_train_count += 1
                    if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc is not None:
                        mpc.set_ebm(ebm)
                        ebm_active = True
                        logger.info(
                            f"[{condition.upper()}] EBM activated | "
                            f"her_buf={len(her_buf)} goal_buf={len(goal_buf)}"
                        )
                ...
                if mpc is None:
                    ...
                    logger.info(f"[{condition.upper()}] CEM ready (H=30) | goals={len(goal_buf)} her={len(her_buf)}")
        ...
        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            logger.info(
                ...
                f"goals={len(goal_buf)} | her={len(her_buf)} | ssl={ssl_ewa or 0.0:.4f} | "
        ...
        if env_step % EVAL_INTERVAL < n_envs:
            ...
            logger.info(
                ...
                f"goals={len(goal_buf)} | her={len(her_buf)} | {elapsed:.0f}s"
            )
...
run_abm_loop = run_doorkey_her_loop
