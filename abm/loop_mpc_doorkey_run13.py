"""
abm/loop_mpc_doorkey_run13.py — Run 13: Frozen V-JEPA 2.1 ViT-Base-384 encoder

Why this over DINOv2 (Run 12):
  DINOv2 is trained on static natural images. V-JEPA 2.1 is trained on ~2M hours
  of video using a JEPA objective — predict future latent representations from
  current ones. This is architecturally closer to what our FeaturePredictor does
  in latent space: z_t → z_{t+1}. The video pretraining makes V-JEPA features
  sensitive to motion, temporal dynamics, and physical causation — exactly what
  DoorKey planning requires (agent moving, door state changing, object pickup).

  V-JEPA 2.1 also explicitly supports single-frame input via img_temporal_dim_size=1
  and interpolate_rope=True, so no temporal hacks required.

Architecture:
  - Frozen V-JEPA 2.1 ViT-Base-384 → mean-pool patch tokens → 768-dim L2-norm features
  - Learned FeaturePredictor: MLP (z, a_oh) → z_next  [trained online]
  - Learned EBMCostHead(768): same contrastive training as before
  - CEMPlanner: same CEM, now in 768-dim V-JEPA space
  - All Run 12 improvements retained:
      FeatureReplayBuffer / GoalFeatureBuffer (features, not raw images)
...
Encoding:
  Input  : (B, 3, 1, 384, 384)  [B, C, T=1, H, W]
  Encoder: V-JEPA 2.1 ViT-B/384 — patch_size=16 → N=576 tokens
  Output : mean_pool((B, 576, 768), dim=1) → (B, 768), L2-normalized
...
N_ENVS                 = 16
N_ACTIONS              = 7
FEATURE_DIM            = 768         # V-JEPA 2.1 ViT-B/384 embed dim
VJEPA2_IMG_SIZE        = 384
PRED_HIDDEN            = 1024
PRED_LR                = 1e-4
PRED_BATCH             = 256
...
def _load_vjepa2(device: str) -> nn.Module:
    ...
    logger.info(f"[RUN13] V-JEPA 2.1 ViT-B/384 loaded — {n_params:.1f}M params, feature_dim={FEATURE_DIM}")
    return encoder
...
def _encode(model: nn.Module, imgs_np: np.ndarray, device: str) -> torch.Tensor:
    ...
    Returns: (B, 768) L2-normalized V-JEPA 2.1 features

    V-JEPA 2.1 input shape: (B, C, T, H, W) with T=1 for single frames.
    Output: (B, N_tokens=576, 768) patch tokens — mean pooled to (B, 768).
    """
    if imgs_np.ndim == 3:
    ...
    if x.shape[-2:] != (VJEPA2_IMG_SIZE, VJEPA2_IMG_SIZE):
        x = F.interpolate(x, size=(VJEPA2_IMG_SIZE, VJEPA2_IMG_SIZE),
                          mode="bilinear", align_corners=False)
    # Add temporal dimension T=1: (B, 3, H, W) → (B, 3, 1, H, W)
    x = x.unsqueeze(2)
    # Normalize with ImageNet stats (same as DINOv2)
    mean = _VJEPA2_MEAN.to(device)
    std  = _VJEPA2_STD.to(device)
    x = (x - mean) / std
    # Forward: (B, 3, 1, 384, 384) → (B, N_tokens, 768)
    tokens = model(x)          # (B, 576, 768)
...
def _doorkey_discrimination_test(model: nn.Module, device: str) -> float:
    """
    Spin up 20 DoorKey envs with different seeds, collect frames, measure
    cross-seed cos_sim. If > 0.95 the features are too similar to plan.
    Returns mean cos_sim across 10 pairs.
    """
    import gymnasium
    from minigrid.wrappers import RGBImgObsWrapper
    ...
    logger.info(
        f"[RUN13] V-JEPA 2.1 DoorKey discrimination test: "
        f"same-frame cos_sim={self_sim:.4f} | "
        f"cross-seed mean cos_sim={mean_sim:.4f}"
    )
    if mean_sim > 0.95:
        logger.warning(
            f"[RUN13] WARNING: cross-seed cos_sim={mean_sim:.4f} > 0.95 — "
            f"features may not be discriminative enough for DoorKey. "
            f"V-JEPA 2.1 was trained on natural video, not synthetic grids. Continuing anyway."
        )
    else:
        logger.info(
            f"[RUN13] Features discriminative (cos_sim={mean_sim:.4f} < 0.95) — proceeding."
        )
    return mean_sim
...
class GoalFeatureBuffer:
    """Circular buffer storing 768-dim features for EBM goal targets."""
...
class FeaturePredictor(nn.Module):
    """
    Learned transition model in V-JEPA 2.1 feature space.
...
def _seed_scripted(
    ...
    logger.info(f"[RUN13] Scripted seeding: {n_eps} episodes…")
    successes = 0
    total_transitions = 0
    ...
    for ep in range(n_eps):
        ...
        while not done and steps < EP_MAX_STEPS:
            ...
            z_prev = _encode(vjepa2, obs["image"], device).cpu().numpy()[0]
            ...
            z_cur = _encode(vjepa2, obs["image"], device).cpu().numpy()[0]
    ...
    logger.info(
        f"[RUN13] Seed done: {successes}/{n_eps} succeeded | "
...
def _eval_run13(
    ...
    for ep in range(n_eps):
        ...
        while not done and ep_steps < EP_MAX_STEPS:
            ...
            z = _encode(vjepa2, obs["image"], device)
...
def run_doorkey_run13_loop(
    condition: str = "vjepa2_frozen",
    device: str = "cuda",
    ...
    if condition != "vjepa2_frozen":
        raise ValueError(f"loop_mpc_doorkey_run13 supports: vjepa2_frozen — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run13.py only supports env_type='doorkey'.")
    ...
    logger.info(
        f"[RUN13] Frozen V-JEPA 2.1 ViT-B/384 + H={CEM_HORIZON} + subgoals + seed_buf + post_door_neg | "
    ...
    # Load V-JEPA 2.1 and run discrimination test before doing anything else
    ...
    while env_step < max_steps:
        ...
        # Encode current observations with frozen V-JEPA 2.1
        z_cur_t = _encode(vjepa2, obs["image"], device)   # (n_envs, 768)
        ...
        z_next_t = _encode(vjepa2, next_obs["image"], device)
        ...
        if final_mask.any() and "final_observation" in infos:
            ...
            for i in range(n_envs):
                if final_mask[i]:
                    z_next_np[i] = _encode(vjepa2, fin_imgs[i], device).cpu().numpy()[0]
        ...
        if pred_loss is not None:
            ...
            if mpc is None and len(buf) >= PRED_WARMUP:
                ...
                logger.info(
                    f"[RUN13] CEM ready (H={CEM_HORIZON}) | "
                    f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}"
                )
        ...
        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            logger.info(
                f"[RUN13] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} "
        ...
        if env_step % EVAL_INTERVAL < n_envs:
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN13] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN13] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run13_loop
