"""
abm/loop_acwm_tiered.py — Tier-aware sticky-goal ACWM scout loop.

This scout keeps the planner-only ACWM structure, but changes the planning
objective for Crafter in two ways:
  - sticky goals: goals persist longer during ACT before being refreshed
  - tier-aware goals: reward-positive states are bucketed by Crafter tier and
    the planner targets the lowest unmastered tier with available examples

It is intended as the next LeCun-aligned objective test after loop_acwm.py.
"""

from __future__ import annotations

import logging
import time
from collections import deque
from pathlib import Path
from typing import Dict, List, Optional, Tuple

import numpy as np
import torch
import torch.optim as optim

from .crafter_env import ACHIEVEMENTS, ACHIEVEMENT_TIERS, make_crafter_env, make_crafter_vec_env
from .lewm import LeWM, ReplayBuffer, SequenceReplayBuffer
from .meta_controller import AutonomousSystemM, FixedSystemM, Mode
from .mpc import CEMPlanner

logger = logging.getLogger(__name__)

N_ENVS = 16
LEWM_LR = 1e-4
LEWM_BATCH = 256
LEWM_WARMUP = 500
FIXED_SWITCH_EVERY = 10_000

TIER_ORDER = tuple(ACHIEVEMENT_TIERS.keys())
TIER_SHORT = {
    "tier1_basic": "t1",
    "tier2_tools": "t2",
    "tier3_advanced": "t3",
    "tier4_hard": "t4",
}
ACHIEVEMENT_TO_TIER = {
    achievement: tier
    for tier, achievements in ACHIEVEMENT_TIERS.items()
    for achievement in achievements
}


def _obs_to_tensor(obs_dict, device: str) -> torch.Tensor:
    imgs = obs_dict["image"]
    if imgs.ndim == 3:
        imgs = imgs[None]
    arr = imgs.astype(np.float32) / 255.0
    return torch.from_numpy(arr).permute(0, 3, 1, 2).to(device)


def _zero_achievement_counts() -> Dict[str, int]:
    return {achievement: 0 for achievement in ACHIEVEMENTS}


class TieredGoalImageBuffer:
    """
    Stores raw goal images in per-tier buckets, with a fallback bucket for
    reward-positive states that could not be labeled from vector-env infos.
    """

    def __init__(
        self,
        capacity_per_tier: int = 512,
        fallback_capacity: int = 256,
    ):
        self._tier_bufs = {
            tier: deque(maxlen=capacity_per_tier) for tier in TIER_ORDER
        }
        self._fallback = deque(maxlen=fallback_capacity)

    def push(self, img: np.ndarray, tiers: Optional[List[str]] = None) -> None:
        img_copy = np.array(img, copy=True)
        unique_tiers = [tier for tier in dict.fromkeys(tiers or []) if tier in self._tier_bufs]
        if not unique_tiers:
            self._fallback.append(img_copy)
            return
        for tier in unique_tiers:
            self._tier_bufs[tier].append(img_copy)

    def tier_count(self, tier: str) -> int:
        return len(self._tier_bufs.get(tier, ()))

    def counts_by_tier(self) -> Dict[str, int]:
        counts = {tier: len(buf) for tier, buf in self._tier_bufs.items()}
        counts["fallback"] = len(self._fallback)
        return counts

    def summary_str(self) -> str:
        counts = self.counts_by_tier()
        parts = [f"{TIER_SHORT[tier]}:{counts[tier]}" for tier in TIER_ORDER]
        parts.append(f"fb:{counts['fallback']}")
        return ",".join(parts)

    def __len__(self) -> int:
        return sum(len(buf) for buf in self._tier_bufs.values()) + len(self._fallback)

    def _sampling_order(self, preferred_tier: Optional[str]) -> List[str]:
        if preferred_tier not in TIER_ORDER:
            return list(TIER_ORDER)
        idx = TIER_ORDER.index(preferred_tier)
        lower = list(reversed(TIER_ORDER[:idx]))
        higher = list(TIER_ORDER[idx + 1 :])
        return [preferred_tier, *lower, *higher]

    def sample_with_tiers(
        self,
        batch_size: int,
        preferred_tier: Optional[str] = None,
    ) -> Optional[Tuple[np.ndarray, List[Optional[str]]]]:
        if len(self) == 0:
            return None

        sampled_imgs: List[np.ndarray] = []
        sampled_tiers: List[Optional[str]] = []
        order = self._sampling_order(preferred_tier)

        for _ in range(batch_size):
            chosen_bucket = None
            chosen_tier = None

            for tier in order:
                buf = self._tier_bufs[tier]
                if buf:
                    chosen_bucket = buf
                    chosen_tier = tier
                    break

            if chosen_bucket is None and self._fallback:
                chosen_bucket = self._fallback
                chosen_tier = None

            if chosen_bucket is None:
                return None

            idx = np.random.choice(len(chosen_bucket))
            sampled_imgs.append(chosen_bucket[idx])
            sampled_tiers.append(chosen_tier)

        return np.stack(sampled_imgs, axis=0), sampled_tiers


def _assign_goal_images(
    active_goal_imgs: List[Optional[np.ndarray]],
    active_goal_tiers: List[Optional[str]],
    refresh_mask: np.ndarray,
    goal_buf: TieredGoalImageBuffer,
    preferred_tier: Optional[str],
) -> None:
    n_refresh = int(refresh_mask.sum())
    if n_refresh == 0:
        return
    sampled = goal_buf.sample_with_tiers(n_refresh, preferred_tier=preferred_tier)
    if sampled is None:
        return
    sampled_imgs, sampled_tiers = sampled
    for slot, env_idx in enumerate(np.flatnonzero(refresh_mask)):
        active_goal_imgs[int(env_idx)] = sampled_imgs[slot]
        active_goal_tiers[int(env_idx)] = sampled_tiers[slot]


def _encode_goal_batch(
    active_goal_imgs: List[Optional[np.ndarray]],
    encoder_fn,
    device: str,
) -> Optional[torch.Tensor]:
    if not active_goal_imgs or any(img is None for img in active_goal_imgs):
        return None
    goal_imgs = np.stack(active_goal_imgs, axis=0)
    with torch.no_grad():
        return encoder_fn({"image": goal_imgs})


def _goal_diversity(goal_buf: TieredGoalImageBuffer, encoder_fn, device: str) -> float:
    if len(goal_buf) < 2:
        return 0.0
    sampled = goal_buf.sample_with_tiers(min(32, len(goal_buf)))
    if sampled is None:
        return 0.0
    imgs, _ = sampled
    with torch.no_grad():
        z = encoder_fn({"image": imgs})
    return z.std(dim=0).mean().item()


def _extract_vector_achievement_counts(infos: dict, n_envs: int) -> List[Dict[str, int]]:
    counts = [dict() for _ in range(n_envs)]
    achievements = infos.get("achievements")
    if not isinstance(achievements, dict):
        return counts

    for achievement in ACHIEVEMENTS:
        values = achievements.get(achievement)
        if values is None:
            continue
        values_arr = np.asarray(values)
        if values_arr.ndim == 0:
            values_arr = np.repeat(values_arr, n_envs)
        for env_idx in range(min(n_envs, len(values_arr))):
            counts[env_idx][achievement] = int(values_arr[env_idx])

    return counts


def _consume_new_tier_unlocks_from_infos(
    infos: dict,
    prev_counts: List[Dict[str, int]],
    dones: np.ndarray,
    n_envs: int,
) -> List[List[str]]:
    current_counts = _extract_vector_achievement_counts(infos, n_envs)
    unlocked_tiers = [[] for _ in range(n_envs)]

    for env_idx in range(n_envs):
        env_prev = prev_counts[env_idx]
        for achievement, current in current_counts[env_idx].items():
            previous = env_prev.get(achievement, 0)
            if current > previous:
                tier = ACHIEVEMENT_TO_TIER.get(achievement)
                if tier is not None:
                    unlocked_tiers[env_idx].append(tier)
            env_prev[achievement] = current

        if dones[env_idx]:
            prev_counts[env_idx] = _zero_achievement_counts()

        unlocked_tiers[env_idx] = list(dict.fromkeys(unlocked_tiers[env_idx]))

    return unlocked_tiers


def _consume_new_tier_unlocks_from_info(
    info: dict,
    prev_counts: Dict[str, int],
) -> List[str]:
    unlocked_tiers: List[str] = []
    achievements = info.get("achievements", {})
    if not isinstance(achievements, dict):
        return unlocked_tiers

    for achievement in ACHIEVEMENTS:
        current = int(achievements.get(achievement, prev_counts.get(achievement, 0)))
        previous = prev_counts.get(achievement, 0)
        if current > previous:
            tier = ACHIEVEMENT_TO_TIER.get(achievement)
            if tier is not None:
                unlocked_tiers.append(tier)
        prev_counts[achievement] = current

    return list(dict.fromkeys(unlocked_tiers))


def _push_goal_state(
    goal_buf: TieredGoalImageBuffer,
    next_img: np.ndarray,
    reward: float,
    unlocked_tiers: Optional[List[str]] = None,
) -> None:
    if reward > 0:
        goal_buf.push(next_img, tiers=unlocked_tiers)


def _tier_scores_from_unlocked(ever_unlocked: Dict[str, int]) -> Dict[str, float]:
    return {
        tier: sum(ever_unlocked.get(achievement, 0) for achievement in achievements) / len(achievements)
        for tier, achievements in ACHIEVEMENT_TIERS.items()
    }


def _choose_target_tier(
    per_tier_scores: Dict[str, float],
    goal_buf: TieredGoalImageBuffer,
    mastery_threshold: float = 0.80,
) -> Optional[str]:
    for tier in TIER_ORDER:
        if goal_buf.tier_count(tier) > 0 and per_tier_scores.get(tier, 0.0) < mastery_threshold:
            return tier
    for tier in TIER_ORDER:
        if goal_buf.tier_count(tier) > 0:
            return tier
    return None


def eval_crafter_acwm_tiered(
    mpc: Optional[CEMPlanner],
    encoder_fn,
    goal_buf: TieredGoalImageBuffer,
    device: str,
    seed_offset: int = 1000,
    n_eps: int = 10,
    goal_refresh_steps: int = 256,
) -> Tuple[float, Dict[str, float]]:
    ever_unlocked: Dict[str, int] = {achievement: 0 for achievement in ACHIEVEMENTS}

    for ep in range(n_eps):
        env = make_crafter_env(seed=seed_offset + ep)
        obs, _ = env.reset(seed=seed_offset + ep)
        done = False
        ep_steps = 0
        goal_age = 0
        prev_counts = _zero_achievement_counts()
        target_tier = _choose_target_tier(_tier_scores_from_unlocked(ever_unlocked), goal_buf)
        goal_sample = goal_buf.sample_with_tiers(1, preferred_tier=target_tier)
        if goal_sample is not None:
            goal_img = goal_sample[0]
        else:
            goal_img = None

        while not done and ep_steps < 1_000:
            with torch.no_grad():
                z = encoder_fn(obs)

            if mpc is not None and goal_img is not None:
                with torch.no_grad():
                    z_goal = encoder_fn({"image": goal_img})
                action = mpc.plan_single(z, z_goal)
            else:
                action = env.action_space.sample()

            next_obs, reward, term, trunc, info = env.step(action)
            done = term or trunc
            ep_steps += 1

            newly_unlocked_tiers = _consume_new_tier_unlocks_from_info(info, prev_counts)
            for achievement, value in info.get("achievements", {}).items():
                if value and achievement in ever_unlocked:
                    ever_unlocked[achievement] = 1

            _push_goal_state(goal_buf, next_obs["image"], reward, newly_unlocked_tiers)

            if reward > 0 or goal_img is None or goal_age >= goal_refresh_steps or done:
                target_tier = _choose_target_tier(_tier_scores_from_unlocked(ever_unlocked), goal_buf)
                goal_sample = goal_buf.sample_with_tiers(1, preferred_tier=target_tier)
                goal_img = goal_sample[0] if goal_sample is not None else None
                goal_age = 0
            else:
                goal_age += 1

            obs = next_obs

        env.close()

    score = sum(ever_unlocked.values()) / len(ACHIEVEMENTS)
    per_tier = _tier_scores_from_unlocked(ever_unlocked)
    return score, per_tier


def run_acwm_tiered_loop(
    condition: str,
    device: str = "cuda",
    max_steps: int = 500_000,
    seed: int = 42,
    n_envs: int = N_ENVS,
    env_type: str = "crafter",
    observe_steps: Optional[int] = None,
    use_mpc: bool = True,
    use_rl: bool = False,
) -> Dict:
    if env_type != "crafter":
        raise ValueError("loop_acwm_tiered.py currently supports env_type='crafter' only.")
    if condition not in ("autonomous", "fixed", "mpc_only", "random"):
        raise ValueError(f"Unsupported ACWM condition: {condition}")
    if use_rl:
        logger.info("[ACWM-TIERED] Ignoring use_rl=True — ACT is planner-only in loop_acwm_tiered.")

    torch.manual_seed(seed)
    np.random.seed(seed)

    img_h = img_w = 64
    n_actions = 17
    latent_dim = 512
    eval_interval = 10_000
    eval_n_eps = 10
    replay_capacity = 300_000
    goal_refresh_steps = 256
    goal_capacity_per_tier = 512
    fallback_goal_capacity = 256
    tier_mastery_threshold = 0.80
    obs_plateau_steps = 30_000
    act_plateau_steps = 30_000
    min_initial_observe = 100_000
    min_sr_to_stay = 0.30
    solve_threshold = 1.01
    n_train_steps = 4
    predictor_type = "transformer"

    if observe_steps is not None:
        min_initial_observe = observe_steps
        logger.info(f"[{condition.upper()}] observe_steps override: min_initial_observe={observe_steps}")

    logger.info(
        f"[{condition.upper()}] Starting ACWM-TIERED — env={env_type}, device={device}, "
        f"max_steps={max_steps}, n_envs={n_envs}, replay={replay_capacity}, "
        f"goal_refresh={goal_refresh_steps}"
    )
    t0 = time.time()

    if device == "cuda":
        torch.backends.cudnn.benchmark = True

    use_async = device == "cuda"
    envs = make_crafter_vec_env(n_envs, seed=seed, use_async=use_async)
    obs, _ = envs.reset(seed=list(range(seed, seed + n_envs)))

    lewm = LeWM(
        latent_dim=latent_dim,
        n_actions=n_actions,
        img_size=img_h,
        predictor_type=predictor_type,
    ).to(device)
    buf_lew = ReplayBuffer(capacity=replay_capacity)
    buf_seq = SequenceReplayBuffer(capacity=replay_capacity, seq_len=8)
    opt_lewm = optim.Adam(lewm.parameters(), lr=LEWM_LR)
    mpc: Optional[CEMPlanner] = None
    goal_buf = TieredGoalImageBuffer(
        capacity_per_tier=goal_capacity_per_tier,
        fallback_capacity=fallback_goal_capacity,
    )

    def encoder_fn(obs_dict):
        return lewm.encode(_obs_to_tensor(obs_dict, device))

    if condition == "autonomous":
        sysm = AutonomousSystemM(
            obs_plateau_steps=obs_plateau_steps,
            act_plateau_steps=act_plateau_steps,
            plateau_threshold=0.03,
            solve_threshold=solve_threshold,
            min_sr_to_stay=min_sr_to_stay,
            min_initial_observe=min_initial_observe,
        )
    elif condition == "fixed":
        sysm = FixedSystemM(
            switch_every=FIXED_SWITCH_EVERY,
            solve_threshold=solve_threshold,
        )
    else:
        sysm = None

    metrics: Dict[str, List] = {
        "env_step": [],
        "success_rate": [],
        "ssl_loss_ewa": [],
        "mode": [],
        "wall_time_s": [],
        "per_tier": [],
    }
    latest_per_tier = {tier: 0.0 for tier in TIER_ORDER}
    current_target_tier: Optional[str] = None
    ssl_ewa = None
    steps_to_80 = None
    env_step = 0
    act_steps = 0
    total_observe_steps = 0
    ep_ret = np.zeros(n_envs, dtype=np.float32)
    last_done = np.zeros(n_envs, dtype=bool)
    prev_achievement_counts = [_zero_achievement_counts() for _ in range(n_envs)]
    active_goal_imgs: List[Optional[np.ndarray]] = [None for _ in range(n_envs)]
    active_goal_tiers: List[Optional[str]] = [None for _ in range(n_envs)]
    goal_ages = np.zeros(n_envs, dtype=np.int32)

    while env_step < max_steps:
        if condition in ("mpc_only", "random"):
            current_mode = Mode.ACT
        elif condition == "autonomous":
            current_mode = sysm.mode
        else:
            current_mode = sysm.step(env_step)

        mode_str = current_mode.name
        executed_mode = current_mode
        ssl_loss_val = None

        if current_mode == Mode.OBSERVE:
            actions = envs.action_space.sample()
            obs_imgs = obs["image"].copy()
            next_obs, rewards, terms, truncs, infos = envs.step(actions)
            next_imgs = next_obs["image"].copy()

            final_mask = infos.get("_final_observation", np.zeros(n_envs, dtype=bool))
            if final_mask.any() and "final_observation" in infos:
                for i in range(n_envs):
                    if final_mask[i]:
                        next_imgs[i] = infos["final_observation"]["image"][i]

            dones = terms | truncs
            unlocked_tiers_by_env = _consume_new_tier_unlocks_from_infos(
                infos,
                prev_achievement_counts,
                dones,
                n_envs,
            )
            for i in range(n_envs):
                buf_lew.push(obs_imgs[i], int(actions[i]), next_imgs[i])
                buf_seq.push(obs_imgs[i], int(actions[i]), bool(dones[i]))
                _push_goal_state(goal_buf, next_imgs[i], float(rewards[i]), unlocked_tiers_by_env[i])

            obs = next_obs
            env_step += n_envs
            total_observe_steps += n_envs

            if len(buf_lew) >= LEWM_WARMUP:
                for _ in range(n_train_steps):
                    obs_t, acts, obs_next = buf_lew.sample(LEWM_BATCH, device)
                    opt_lewm.zero_grad()
                    loss, info = lewm.loss(obs_t, acts, obs_next)
                    loss.backward()
                    opt_lewm.step()
                ssl_loss_val = info["loss_total"]

                if len(buf_seq) > buf_seq.seq_len * 2:
                    seq_data = buf_seq.sample_sequences(
                        batch_size=64,
                        encoder_fn=lewm.encode,
                        n_actions=n_actions,
                        device=device,
                    )
                    if seq_data is not None:
                        z_seq, a_onehot_seq, z_next_target = seq_data
                        opt_lewm.zero_grad()
                        z_pred = lewm.predictor.forward_sequence(z_seq, a_onehot_seq)
                        seq_loss = (1 - torch.nn.functional.cosine_similarity(
                            z_pred,
                            z_next_target.detach(),
                            dim=-1,
                        )).mean()
                        seq_loss.backward()
                        opt_lewm.step()

                ssl_ewa = ssl_loss_val if ssl_ewa is None else 0.95 * ssl_ewa + 0.05 * ssl_loss_val

                if condition == "autonomous":
                    observe_signal = ssl_ewa if ssl_ewa is not None else ssl_loss_val
                    sysm.observe_step(observe_signal, env_step)

                if mpc is None:
                    mpc = CEMPlanner(
                        lewm.predictor,
                        n_actions=n_actions,
                        horizon=15,
                        n_samples=256,
                        n_elites=32,
                        n_iters=4,
                        device=device,
                        distance="cosine",
                    )
                    logger.info(
                        f"[{condition.upper()}] ACWM-TIERED planner ready for Crafter "
                        f"(horizon=15, samples=256, elites=32, iters=4)"
                    )

                current_target_tier = _choose_target_tier(
                    latest_per_tier,
                    goal_buf,
                    mastery_threshold=tier_mastery_threshold,
                )

                if (
                    condition == "autonomous"
                    and sysm.mode == Mode.OBSERVE
                    and sysm.n_switches() == 0
                    and env_step >= min_initial_observe
                    and mpc is not None
                    and len(goal_buf) > 0
                ):
                    sysm.force_switch(
                        Mode.ACT,
                        env_step,
                        "initial observe budget satisfied; planner ready; tiered goals available",
                    )

        else:
            current_target_tier = _choose_target_tier(
                latest_per_tier,
                goal_buf,
                mastery_threshold=tier_mastery_threshold,
            )
            refresh_mask = last_done.copy() | (goal_ages >= goal_refresh_steps)
            refresh_mask |= np.array([img is None for img in active_goal_imgs], dtype=bool)
            if len(goal_buf) > 0:
                _assign_goal_images(
                    active_goal_imgs,
                    active_goal_tiers,
                    refresh_mask,
                    goal_buf,
                    preferred_tier=current_target_tier,
                )

            if mpc is not None and len(goal_buf) > 0 and condition != "random":
                with torch.no_grad():
                    z = encoder_fn(obs)
                    z_goal = _encode_goal_batch(active_goal_imgs, encoder_fn, device)
                if z_goal is not None:
                    actions_np = mpc.plan_batch(z, z_goal)
                else:
                    actions_np = envs.action_space.sample()
            else:
                actions_np = envs.action_space.sample()

            obs_imgs = obs["image"].copy()
            next_obs, rewards, terms, truncs, infos = envs.step(actions_np)
            next_imgs = next_obs["image"].copy()

            final_mask = infos.get("_final_observation", np.zeros(n_envs, dtype=bool))
            if final_mask.any() and "final_observation" in infos:
                for i in range(n_envs):
                    if final_mask[i]:
                        next_imgs[i] = infos["final_observation"]["image"][i]

            dones = terms | truncs
            unlocked_tiers_by_env = _consume_new_tier_unlocks_from_infos(
                infos,
                prev_achievement_counts,
                dones,
                n_envs,
            )
            ep_ret += rewards
            env_step += n_envs
            act_steps += n_envs

            for i in range(n_envs):
                buf_lew.push(obs_imgs[i], int(actions_np[i]), next_imgs[i])
                buf_seq.push(obs_imgs[i], int(actions_np[i]), bool(dones[i]))
                _push_goal_state(goal_buf, next_imgs[i], float(rewards[i]), unlocked_tiers_by_env[i])

                goal_ages[i] += 1
                if rewards[i] > 0 or dones[i]:
                    active_goal_imgs[i] = None
                    active_goal_tiers[i] = None
                    goal_ages[i] = 0

                if dones[i]:
                    if condition == "autonomous":
                        sysm.act_step(float(ep_ret[i]), None, env_step)
                    ep_ret[i] = 0.0

            last_done = dones
            obs = next_obs

        if env_step % eval_interval < n_envs:
            sr, per_tier = eval_crafter_acwm_tiered(
                mpc=mpc,
                encoder_fn=encoder_fn,
                goal_buf=goal_buf,
                device=device,
                seed_offset=9000 + env_step,
                n_eps=eval_n_eps,
                goal_refresh_steps=goal_refresh_steps,
            )
            latest_per_tier = per_tier
            current_target_tier = _choose_target_tier(
                latest_per_tier,
                goal_buf,
                mastery_threshold=tier_mastery_threshold,
            )

            if condition == "autonomous" and executed_mode == Mode.ACT:
                sysm.act_step(None, sr, env_step)

            if sysm is not None:
                mode_str = sysm.mode.name

            n_sw = sysm.n_switches() if sysm else 0
            elapsed = time.time() - t0
            logger.info(
                f"[{condition.upper()}] step={env_step:7d} | mode={mode_str:12s} | "
                f"success={sr:.1%} | ssl_ewa={ssl_ewa or 0.0:.4f} | "
                f"act_steps={act_steps:7d} | obs_steps={total_observe_steps:7d} | "
                f"switches={n_sw} | tiers={per_tier} | {elapsed:.0f}s"
            )

            detail_parts = [
                f"replay={len(buf_lew)}",
                f"goals={len(goal_buf)}",
                f"goal_counts={goal_buf.summary_str()}",
                f"target_tier={current_target_tier or 'fallback'}",
                f"active_goals={sum(img is not None for img in active_goal_imgs)}",
            ]
            if mpc is not None:
                detail_parts.append("mpc=ready")
                detail_parts.append(f"cem_cost={getattr(mpc, '_last_best_cost', 0.0):.4f}")
            else:
                detail_parts.append("mpc=NOT_READY")
            if ssl_loss_val is not None:
                detail_parts.append(f"wm_loss={ssl_loss_val:.4f}")
            if len(goal_buf) > 1:
                detail_parts.append(f"goal_div={_goal_diversity(goal_buf, encoder_fn, device):.4f}")
            if condition == "autonomous" and sysm is not None:
                detail_parts.append(
                    f"sysm(ssl_buf={len(sysm._ssl_buf)},sr_buf={len(sysm._sr_buf)},time_in={env_step - sysm._mode_start})"
                )
            logger.info(f"  details: {' | '.join(detail_parts)}")

            metrics["env_step"].append(env_step)
            metrics["success_rate"].append(sr)
            metrics["ssl_loss_ewa"].append(ssl_ewa or 0.0)
            metrics["mode"].append(mode_str)
            metrics["wall_time_s"].append(elapsed)
            metrics["per_tier"].append(per_tier)

            if steps_to_80 is None and sr >= 0.80:
                steps_to_80 = env_step
                logger.info(f"[{condition.upper()}] *** 80% at step {env_step} ***")

    envs.close()
    elapsed_total = time.time() - t0

    ckpt_dir = Path("results/crafter")
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    ckpt_path = ckpt_dir / f"checkpoint_{condition}_acwm_tiered.pt"
    torch.save(
        {
            "condition": condition,
            "env_type": env_type,
            "env_step": env_step,
            "latent_dim": latent_dim,
            "n_actions": n_actions,
            "replay_capacity": replay_capacity,
            "goal_capacity_per_tier": goal_capacity_per_tier,
            "fallback_goal_capacity": fallback_goal_capacity,
            "goal_refresh_steps": goal_refresh_steps,
            "goal_strategy": "tiered_sticky",
            "lewm": lewm.state_dict(),
        },
        ckpt_path,
    )
    logger.info(f"[{condition.upper()}] ACWM-TIERED checkpoint → {ckpt_path}")

    return {
        "condition": condition,
        "env_type": env_type,
        "env_steps": metrics["env_step"],
        "success_rate": metrics["success_rate"],
        "ssl_loss_ewa": metrics["ssl_loss_ewa"],
        "mode": metrics["mode"],
        "wall_time_s": metrics["wall_time_s"],
        "per_tier": metrics["per_tier"],
        "steps_to_80pct": steps_to_80,
        "n_switches": sysm.n_switches() if sysm else 0,
        "switch_log": sysm.switch_log if sysm else [],
        "total_time_s": elapsed_total,
        "act_steps": act_steps,
        "observe_steps": total_observe_steps,
        "goal_strategy": "tiered_sticky",
    }


run_abm_loop = run_acwm_tiered_loop
