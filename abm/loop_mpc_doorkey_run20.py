"""
abm/loop_mpc_doorkey_run20.py — Run 20: Two-phase EBM (hinge → softplus)

Why this run:
  Run 15c: hinge margin=1, sharp early discrimination (83% door→exit
  conversion), but saturates at ~85k ACT when margins are satisfied.
  Run 18: softplus throughout, never saturates, but gradient = sigmoid(·) ≤ 1
  gives weaker early discrimination (~14% conversion). Peak=25%, final=5%.
  Run 19: hinge margin=10, learning target too far — EBM never builds
  useful discrimination. Effectively 0% conversion.

  Run 20 combines both strengths:
    OBSERVE (0→40k):  hinge margin=1 — fast, sharp EBM discrimination.
                      Same as Run 15c. Enters ACT with a well-trained EBM.
    ACT (40k→200k):   softplus — gradient never dies. Maintains the sharp
                      discrimination built in OBSERVE without ever saturating.

  Expected: Run 15c's 83% early conversion that doesn't fall off at 85k ACT.
...
def _seed_scripted(key_buf, door_buf, goal_buf, seed_buf, post_door_neg_buf,
    ...
    logger.info(f"[RUN20] Scripted seeding: {n_eps} episodes (5-dim symbolic)…")
    successes = 0
    ...
    logger.info(
        f"[RUN20] Seed done: {successes}/{n_eps} | seed_buf={len(seed_buf)} | "
...
def _eval_run20(mpc, key_buf, door_buf, goal_buf, device, seed_offset=1000, n_eps=EVAL_N_EPS):
    if mpc is None:
        return 0.0
    successes = 0
    for ep in range(n_eps):
...
def run_doorkey_run20_loop(
    condition: str = "symbolic_two_phase_ebm",
    device: str = "cuda",
    ...
    if condition != "symbolic_two_phase_ebm":
        raise ValueError(f"loop_mpc_doorkey_run20 supports: symbolic_two_phase_ebm — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run20 only supports env_type='doorkey'.")

    torch.manual_seed(seed); np.random.seed(seed)
    _observe_steps = observe_steps if observe_steps is not None else DEFAULT_OBSERVE

    logger.info(
        f"[RUN20] Symbolic (5-dim) + two-phase EBM (hinge→softplus at ACT) | "
        ...
        if pred_loss is not None:
            ...
            if train_fn(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf, post_door_neg_buf, buf, device):
                ...
                if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc is not None:
                    ...
                    logger.info(f"[RUN20] EBM activated (hinge→softplus at step {env_step}) | "
            ...
            if mpc is None and len(buf) >= PRED_WARMUP:
                ...
                logger.info(f"[RUN20] CEM ready (H={CEM_HORIZON}, dim={FEATURE_DIM}) | "
                            f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")

        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            logger.info(
                f"[RUN20] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} "
                f"post_neg={len(post_door_neg_buf)} | "
                f"pred_ewa={pred_ewa or 0.0:.4f} | "
                f"ebm={'ON(softplus)' if (ebm_active and not in_observe) else 'ON(hinge)' if ebm_active else f'training({ebm_train_count})'} | {time.time()-t0:.0f}s"
            )

        if env_step % EVAL_INTERVAL < n_envs:
            sr = _eval_run20(mpc, key_buf, door_buf, goal_buf, device,
                              seed_offset=9000 + env_step, n_eps=EVAL_N_EPS)
            elapsed = time.time() - t0
            logger.info(
                f"[RUN20] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN20] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN20] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run20_loop
