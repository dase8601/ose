"""
abm/loop_mpc_doorkey_run21.py — Run 21: H=12 for stage 2 (door→exit)

Why this run:
  Runs 15c, 18, and 20 all stall at goal≈224–226 despite different EBM
  loss functions.  The EBM and predictor are both healthy (pred_ewa 0.013+
  in Run 20). Three independent architectures hitting the same ceiling rules
  out saturation — it is a horizon bottleneck.

  In MiniGrid-DoorKey-6x6 the agent must navigate from the door cell to the
  exit cell in the lower-right corner.  With H=8 and n_envs=16, easy layouts
  (short door→exit path) are solved; hard layouts (door far from exit, need
  10+ steps) are never reachable by the CEM rollout.

  Run 21 keeps every design decision of Run 20 intact and makes exactly one
  change: stage-2 planning uses H=12 (up from H=8).  Stages 0/1 keep H=8
  (proven sufficient — key and door subgoals are short-range).
...
CEM_HORIZON_S01        = 8    # stages 0/1: key pickup + door open (proven)
CEM_HORIZON_S2         = 12   # stage 2: door→exit (targeted fix)
CEM_SAMPLES            = 512
CEM_ELITES             = 64
CEM_ITERS              = 5
...
def _seed_scripted(key_buf, door_buf, goal_buf, seed_buf, post_door_neg_buf,
    ...
    logger.info(f"[RUN21] Scripted seeding: {n_eps} episodes (5-dim symbolic)…")
    successes = 0
    ...
    logger.info(
        f"[RUN21] Seed done: {successes}/{n_eps} | seed_buf={len(seed_buf)} | "
...
def _train_ebm_softplus(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf,
    ...
    """ACT-phase EBM training: softplus(E_pos - E_neg) — gradient never zero."""
    if len(buf) < EBM_BATCH * 2:
        return False
    opt_ebm.zero_grad()
    total_loss = torch.tensor(0.0, device=device)
    ...
    if len(her_buf) >= EBM_HER_MIN:
        ...
        e_pos = ebm(z_her, z_g_her)
        e_neg = ebm(z_n2,  z_g_her)
        total_loss = total_loss + F.softplus(e_pos - e_neg).mean(); n_terms += 1
    ...
    if len(goal_buf) >= EBM_MIN_GOALS and len(post_door_neg_buf) >= EBM_POST_DOOR_MIN:
        ...
        z_g_exit = _z_from_buf(goal_buf, EBM_BATCH, device)
        z_rh_neg = _z_from_buf(post_door_neg_buf, EBM_BATCH, device)
        e_pos = ebm(z_exit,   z_g_exit)
        e_neg = ebm(z_rh_neg, z_g_exit)
        total_loss = total_loss + F.softplus(e_pos - e_neg).mean(); n_terms += 1
...
def _eval_run21(mpc_fast, mpc_deep, key_buf, door_buf, goal_buf, device,
    ...
    for ep in range(n_eps):
        ...
        while not done and ep_steps < EP_MAX_STEPS:
            ...
            planner = mpc_deep if current_stage == 2 else mpc_fast
...
def run_doorkey_run21_loop(
    condition: str = "symbolic_horizon12_s2",
    device: str = "cuda",
    ...
    if condition != "symbolic_horizon12_s2":
        raise ValueError(f"loop_mpc_doorkey_run21 supports: symbolic_horizon12_s2 — got: {condition}")
    if env_type != "doorkey":
        raise ValueError("loop_mpc_doorkey_run21 only supports env_type='doorkey'.")

    torch.manual_seed(seed); np.random.seed(seed)
    _observe_steps = observe_steps if observe_steps is not None else DEFAULT_OBSERVE

    logger.info(
        f"[RUN21] Symbolic (5-dim) + two-phase EBM + H_s01={CEM_HORIZON_S01} H_s2={CEM_HORIZON_S2} | "
    ...
    mpc_fast: Optional[CEMPlanner] = None   # H=8 for stages 0/1
    mpc_deep: Optional[CEMPlanner] = None   # H=12 for stage 2
    ...
    while env_step < max_steps:
        ...
        if in_observe:
            ...
            if mpc_fast is not None:
                # Split envs by stage — stage 2 gets the longer-horizon planner
                s01_idx = [i for i in range(n_envs)
                           if active_stage[i] < 2 and active_goal_z[i] is not None]
                s2_idx  = [i for i in range(n_envs)
                           if active_stage[i] == 2 and active_goal_z[i] is not None]
                no_goal = [i for i in range(n_envs) if active_goal_z[i] is None]
                ...
                if s01_idx:
                    ...
                    actions[s01_idx] = mpc_fast.plan_batch(z_s01, g_s01)

                if s2_idx:
                    z_s2 = z_cur_t[s2_idx]
                    g_s2 = torch.from_numpy(
                        np.stack([active_goal_z[i] for i in s2_idx])
                    ).to(device)
                    actions[s2_idx] = mpc_deep.plan_batch(z_s2, g_s2)

                for i in no_goal:
                    actions[i] = envs.envs[i].action_space.sample()
            else:
                actions = envs.action_space.sample()
        ...
        if pred_loss is not None:
            ...
            if train_fn(ebm, opt_ebm, key_buf, door_buf, goal_buf, her_buf, post_door_neg_buf, buf, device):
                ebm_train_count += 1
                if ebm_train_count >= EBM_WARMUP_STEPS and not ebm_active and mpc_fast is not None:
                    mpc_fast.set_ebm(ebm); mpc_deep.set_ebm(ebm)
                    ebm_active = True
                    logger.info(f"[RUN21] EBM activated (hinge→softplus at step {env_step}) | "
                                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")

            if mpc_fast is None and len(buf) >= PRED_WARMUP:
                mpc_fast = CEMPlanner(predictor, n_actions=N_ACTIONS, horizon=CEM_HORIZON_S01,
                                      n_samples=CEM_SAMPLES, n_elites=CEM_ELITES, n_iters=CEM_ITERS,
                                      device=device, distance="cosine")
                mpc_deep = CEMPlanner(predictor, n_actions=N_ACTIONS, horizon=CEM_HORIZON_S2,
                                      n_samples=CEM_SAMPLES, n_elites=CEM_ELITES, n_iters=CEM_ITERS,
                                      device=device, distance="cosine")
                logger.info(f"[RUN21] CEM ready (H_s01={CEM_HORIZON_S01}, H_s2={CEM_HORIZON_S2}, dim={FEATURE_DIM}) | "
                            f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)}")
        ...
        if (env_step // n_envs) % 1000 == 0 and env_step > 0:
            ...
            logger.info(
                f"[RUN21] heartbeat step={env_step} | buf={len(buf)} seed_buf={len(seed_buf)} | "
                f"key={len(key_buf)} door={len(door_buf)} goal={len(goal_buf)} "
                f"post_neg={len(post_door_neg_buf)} | "
                f"pred_ewa={pred_ewa or 0.0:.4f} | ebm={ebm_status} | {time.time()-t0:.0f}s"
            )

        if env_step % EVAL_INTERVAL < n_envs:
            sr = _eval_run21(mpc_fast, mpc_deep, key_buf, door_buf, goal_buf, device,
                              seed_offset=9000 + env_step, n_eps=EVAL_N_EPS)
            elapsed = time.time() - t0
            logger.info(
                f"[RUN21] step={env_step:7d} | mode={mode_str:7s} | "
            ...
            if steps_to_80 is None and sr >= 0.80:
                ...
                logger.info(f"[RUN21] *** 80% at step {env_step} ***")
    ...
    logger.info(f"[RUN21] Done | total_time={elapsed_total:.0f}s")
...
run_abm_loop = run_doorkey_run21_loop
